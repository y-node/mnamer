...
async function main() {
  ...
  const filePaths = await walkPaths(args.sources, args.recursive, args.target);
  ...
  for (const file of filePaths) {
    ...
    const targetLocation = args.target ? path.resolve(args.target) : path.dirname(file);
    console.log(`${file} -> ${path.join(targetLocation, newBasename)}`);
    try {
